odoo.define("cimarlab_demo_portal.animation", function (require) {
    "use strict";

    var sAnimation = require("website.content.snippets.animation");

    sAnimation.registry.cimarlab_demo_portal = sAnimation.Class.extend({
        selector: ".o_cimarlab_demo_portal",

        start: function () {
            var self = this;
            var def = this._rpc({
                route: "/cimarlab_demo_portal/helloworld",
            }).then(function (data) {
                if (data.error) {
                    return;
                }
                ...
                if (data["demo_boolean"]) {
                    self.$(".demo_boolean_value").text(data["demo_boolean"]);
                }
                if (data["demo_char"]) {
                ...
                if (data["demo_integer"]) {
                    self.$(".demo_integer_value").text(data["demo_integer"]);
                }
